...
def build_reasoned_prompt(query: str, classification: Dict, is_case_with_sources: bool = False) -> str:
    ...
    if classification['intent'] == 'legal_case' and is_case_with_sources:
        ...
        base_prompt += "3. Take your time to craft a very detailed, professional, irresistible, and perfect draft tailored to Nigerian legal standards. Make it comprehensive, with precise language, all necessary clauses, and impeccable structure. At the end, state that consult lawyers for legal advise, get intouch with a lawyer at Lawyernearme.com.ng today.\n"
    else:
...
GENERAL_ANSWERS = {
    ...
    "who created you": "I was developed by Best-lab2000 to assist with legal research and explanations.",
...
async def ask_question(request: Request):
    ...
    async def generate():
        ...
        # === FULL HISTORY FOR SAVING / DISPLAY (kept complete) ===
        history: List[Dict] = []
        if chat_id:
            chat_data = load_chat_history(chat_id)
            if chat_data:
                history = chat_data.get("history", []).copy()

        if not chat_id:
            chat_id = str(uuid.uuid4())

        # === MESSAGES FOR MODEL — ONLY LAST ASSISTANT MESSAGE AS CONTEXT (minimal token usage) ===
        messages = [
            {"role": "system", "content": "You are JuristMind, a logical AI specializing in Nigerian law but capable of answering general queries. For law-related answers, quote specific statutes and cases."}
        ]
        if history and history[-1]["role"] == "assistant":
            messages.append(history[-1].copy())

        user_msg = {"role": "user", "content": question}
        messages.append(user_msg)
        history.append(user_msg.copy())  # keep full history for saving

        if general_answer:
            sanitized = sanitize_response(general_answer)
            assistant_msg = {"role": "assistant", "content": sanitized}
            history.append(assistant_msg.copy())
            full_text = sanitized
            yield f"data: {json.dumps({'content': sanitized})}\n\n"

        else:
            # Draft template prefix
            prefix = ""
            suffix = ""
            if classification['intent'] == 'draft':
            ...
            if classification['intent'] == 'legal_case':
                ...
                custom_prompt = build_reasoned_prompt(question, classification, is_case_with_sources=True)
            else:
                ...
                custom_prompt = build_reasoned_prompt(question, classification)
        ...
        # Save full history (only if no error)
        if not has_error:
